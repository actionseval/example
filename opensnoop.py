...
# 14-Aug-2025   Javid Ditty     Remove args.exec, only print (full) paths
...
parser.add_argument("repository",
    help="owner and repository name (e.g. octocat/Hello-World)")
...
used_paths = set()
working_directory = f'/home/runner/work/{args.repository}/'
...
def print_event(cpu, data, size):
    ...
    if not skip:
        ...
        if result not in used_paths and result.startswith(working_directory):
            relative_result = result.removeprefix(working_directory)
            printb(b"%s" % relative_result.encode("utf-8"))
            used_paths.add(relative_result)
        # else:
